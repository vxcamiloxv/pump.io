        ...
        function(err, activities) {
            ...
            if (_.isArray(activities)) {
                activities.forEach(function(shareAct) {
                    // This is AS2
                    shareAct.object.objectType = "Tombstone";
                    shareAct.object.deleted = Stamper.stamp();
                    shareAct.save(group());
                });
            } else {
                this();
...
Activity.prototype.removeStreams = function(callback) {
    var obj = this,
        User = require("./user").User;

    Step(
        function() {
            User.fromPerson(obj.actor.id, this);
        },
        function(err, user) {
            if (err) throw err;
            user.removeFromInbox(obj, this.parallel());
            user.removeFromOutbox(obj, this.parallel());
        },
        callback
    );
};
...
Activity.prototype.beforeEfface = function(callback) {
    this.removeStreams(callback);
};
...
Activity.prototype.afterEfface = function(callback) {
    callback(null);
};
